//META{"name":"SunucuBilgi","website":"https://metalloriff.github.io/toms-discord-stuff/","source":"https://github.com/Metalloriff/BetterDiscordPlugins/blob/master/DetailedServerTooltips.plugin.js"}*//

class SunucuBilgi {

	getName() { return "SunucuBilgi"; }
	getDescription() { return "Detaylı Sunucu Bilgilerini Gösterir."; }
	getVersion() { return "0.3.12"; }
	getAuthor() { return "SekoMirson"; }
	getChanges() {
		return {
			"0.1.1": `
		...
		return {
			...
			preview: { type: "custom", html: `
					<div class="tooltip tooltip-right dst-tooltip" style="position: relative; margin-top: 20px;">
							...
							<div id="dst-tooltip-owner-label" class="dst-tooltip-label">Sahibi: KappaSeko#1995</div>
							<div class="dst-tooltip-label">Joined at: ${new Date().toLocaleDateString()}, ${new Date().toLocaleTimeString()} (0 days ago)</div>
							<div id="dst-tooltip-member-count-label" class="dst-tooltip-label">400 Üye</div>
							<div class="dst-tooltip-label">15 Kanal</div>
							<div class="dst-tooltip-label">10 Röl</div>
							<div class="dst-tooltip-label">Bölge: us-central</div>
							<div style="font-weight: bolder;" class="dst-tooltip-label"><div class="profileBadgePartner-SjK6L2 profileBadge-2BqF-Z" style="display: inline-block;"></div>Discord Ortağı</div>
					</div>
			` },
			minimalMode: { label: "Küçük modu", type: "bool" },
	...
	tooltip(guildId, element) {
		...
		tooltip.innerHTML = `${this.escapeHtml(guild.name)}
				...
				<div id="dst-tooltip-owner-label" class="dst-tooltip-label">Sahibi: ${owner ? this.escapeHtml(owner.tag) : "unknown"}</div>
				<div class="dst-tooltip-label">Oluşturuldu: ${creationDate.toLocaleDateString()}, ${creationDate.toLocaleTimeString()} (${Math.round(Math.abs(creationDate.getTime() - new Date().getTime()) / 86400000)} gün önce)</div>
				${creationDate.toString() == guild.joinedAt.toString() ? "" : `<div class="dst-tooltip-label">Katıldın: ${guild.joinedAt.toLocaleDateString()}, ${guild.joinedAt.toLocaleTimeString()} (${Math.round(Math.abs(guild.joinedAt.getTime() - new Date().getTime()) / 86400000)} gün önce)</div>`}
				<div id="dst-tooltip-member-count-label" class="dst-tooltip-label">${this.memberCountModule.getMemberCount(guildId)} Üye</div>
				<div class="dst-tooltip-label">${Object.values(this.channelModule.getChannels()).filter(c => c.guild_id == guildId).length} Kanal</div>
				<div class="dst-tooltip-label">${Object.keys(guild.roles).length} Röl</div>
				<div class="dst-tooltip-label">Bölge: ${guild.region}</div>`;
		...
		const self = setInterval(() => {
			...
			document.getElementById("dst-tooltip-owner-label").innerHTML = "Sahibi: " + (this.escapeHtml(this.owners[guildId] || "unknown"));
